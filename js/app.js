    ...
    app.factory('Service', ['$http','$location', function($http,$location){
            var api = {
                login : function(scope) {  
                     return $http.get('http://192.168.43.51:8080/DiaryS/rest/api/userlog/'+scope.data.username+"/"+scope.data.password)
             ...
             console.log(user);
                     ...
                     return $http.get('http://192.168.43.51:8080/DiaryS/rest/api/registerwa/'+scope.data.username+"/"+scope.data.password)
                     .success(function(data) {
                        if (data == "success") {
                            scope.commonData.state = 100;
                            scope.commonData.user = {
          ...
          error(function(data) {
             console.log(user);
                     ...
                     return $http.get('http://192.168.43.51:8080/DiaryS/rest/api/getallDiary/'+scope.commonData.user.username)
                     .success(function(data) {
                    scope.diaries = data;
          ...
          error(function(data) {
             // console.log(user);
                     ...
                     return $http.get('http://192.168.43.51:8080/DiaryS/rest/api/deletewa/'+diary_id)
                     .success(function(data) {
                    console.log(scope.diaries);
...
console.log(scope.diaries);
            angular.forEach(scope.diaries, function (item, key) {
                if (item.id == diary_id) {
                    ...
                    return;
                             ...
                             console.log('http://192.168.43.51:8080/DiaryS/rest/api/creatediarywa/'
                        +scope.commonData.user.username+"/"+scope.data.title
                        +"/"+scope.data.blog);
 
                     return $http.get('http://192.168.43.51:8080/DiaryS/rest/api/creatediarywa/'
                        +scope.commonData.user.username+"/"+scope.data.title
                        +"/"+scope.data.blog)
                     .success(function(data) {
                   $location.path('/content');
    ...
    app.controller('registrationController', 
        ...
        $scope.register = function(){
            console.log('http://192.168.43.51:8080/DiaryS/rest/api/registerwa/'+$scope.data.username+"/"+$scope.data.password);
